...
router.get('/list',(req,res)=>{
  ...
  var sql1='SELECT COUNT(*) AS a FROM wmsc_laptop';
  if($kw){
  ...
  var sql2 = `SELECT lid,title,price,sold_count,is_onsale FROM wmsc_laptop ${$kw ? 'WHERE title LIKE "%'+$kw+'%"':''} ORDER BY sold_count DESC LIMIT ${start},${count};`;
  //执行SQL语句，响应查询到的数据
  pool.query(`${sql1};${sql2}`,(err,result)=>{
    ...
    for(var i=0;i<output.data.length;i++){
      ...
      (function(lid,i){
        pool.query('SELECT md FROM wmsc_laptop_pic WHERE laptop_id=? LIMIT 0,1',[lid],(err,result)=>{
...
router.get('/detail',(req,res)=>{
  ...
  pool.query('SELECT * FROM wmsc_laptop WHERE lid=?',[$lid],(err,result)=>{
    if(err) throw err;
    //判断数据是否为空
	//console.log(result)
    if(result.length==0){
	  ...
	  console.log(fid)
		  var pid=lid
      var sql=`
      SELECT * FROM wmsc_laptop_pic WHERE pid=?;
      SELECT * FROM wmsc_laptop_family WHERE fid=${fid};
      SELECT lid,spec FROM wmsc_laptop WHERE family_id=${fid};
      `;
...
router.get('/delete',(req,res)=>{
  ...
  //执行SQL语句
  pool.query('DELETE FROM wmsc_laptop WHERE lid=?',[$lid],(err,result)=>{
	if(err) throw err;
...
router.post('/add',(req,res)=>{
  ...
  //执行SQL语句
  pool.query('INSERT INTO wmsc_laptop SET ?',[obj],(err,result)=>{
    if(err) throw err;
	//console.log(result);
	if(result.affectedRows>0){
