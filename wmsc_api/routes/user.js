...
router.post('/register',(req,res)=>{
  ...
  pool.query('INSERT INTO wmsc_user SET ?',[obj],(err,result)=>{
    if(err) throw err;
	//是否添加成功
...
router.post('/login',(req,res)=>{
  ...
  pool.query('SELECT * FROM wmsc_user WHERE uname=? AND upwd=?',[$uname,$upwd],(err,result)=>{
    if(err) throw err;
	//判断查询的结果（数组）长度是否大于0
...
router.get('/detail',(req,res)=>{
  ...
  //响应查询到的用户对象
  pool.query('SELECT * FROM wmsc_user WHERE uid=?',[$uid],(err,result)=>{
    if(err) throw err;
	//如何判断是否检索到了用户
	//判断结果（数组）长度是否大于0
	if(result.length>0){
...
router.post('/update',(req,res)=>{
  ...
  //执行SQL语句
  pool.query('UPDATE wmsc_user SET email=?,phone=?,gender=?,user_name=? WHERE uid=?',[$email,$phone,$gender,$user_name,$uid],(err,result)=>{
    if(err) throw err;
	//判断是否更改成功
	if(result.affectedRows>0){
...
router.get('/list',(req,res)=>{
  ...
  var sql1='SELECT COUNT(*) AS a FROM wmsc_user';
  if($kw){
...
router.get('/delete',(req,res)=>{
  ...
  //执行SQL语句，删除对应的数据
  pool.query('DELETE FROM wmsc_user WHERE uid=?',[$uid],(err,result)=>{
    if(err) throw err;
    //判断是否删除成功
    if(result.affectedRows>0){
...
router.get('/checkemail',(req,res)=>{
  ...
  pool.query('SELECT uid FROM wmsc_user WHERE email=? LIMIT 1',$email,(err,result)=>{
    if(err) throw err;
    if(result.length>0){
...
router.get('/checkphone',(req,res)=>{
  ...
  pool.query('SELECT uid FROM wmsc_user WHERE phone=? LIMIT 1',$phone,(err,result)=>{
    if(err) throw err;
    if(result.length>0){
      res.send({code:201,msg:'exists'});
    }else{
...
router.get('/checkuname',(req,res)=>{
  ...
  pool.query('SELECT uid FROM wmsc_user WHERE uname=? LIMIT 1',$uname,(err,result)=>{
    if(err) throw err;
    if(result.length>0){
      res.send({code:201,msg:'exists'});
    }else{
